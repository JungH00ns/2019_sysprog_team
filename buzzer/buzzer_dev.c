...
#define BUZZER_DEV_NAME   "buzzer_pwm"
...
int init_pwm(void) {
   ...
   *clkctl = BCM_PASSWORD | (0x01 << 5); // stop PWM Clock
   ...
   int idiv = (int)(330); // Oscilloscope to 16kHz
...
int buzzer_open(struct inode *inode, struct file *filp){
   ...
   gpsel4=(volatile unsigned int *)(gpio_base+GPFSEL4);
...
long buzzer_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
   ...
   switch(cmd) {
      ...
      case IOCTL_CMD_CLEAR_DIRECTION: 
         printk(KERN_ALERT "buzzer clear direction out!!\n");
         *pwmdat1&= ~(1<<5);
         *pwmdat1|= (1<<4);   	 	
